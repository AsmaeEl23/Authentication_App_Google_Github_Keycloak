package ma.sdia.customerfrontthymeleafapplication.web;
...
public class CustomerController {
    ...
    @PreAuthorize("hasAuthority('ADMIN')") //for the role admin
    ...
    public String oauth2Login(Model model) {
        //looking for the registered clients
        String authorizationRequestBaseUri = "oauth2/authorization";
        Map<String, String> oauth2AuthenticationUrls = new HashMap();
        Iterable<ClientRegistration> clientRegistrations = (Iterable<ClientRegistration>) clientRegistrationRepository;;
        clientRegistrations.forEach(registration ->{
            oauth2AuthenticationUrls.put(
                    registration.getClientName(), //key
                    authorizationRequestBaseUri + "/" + registration.getRegistrationId() //value
            );
        });
